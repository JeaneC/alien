...
def get_number_rows(ai_settings, ship_height, alien_height):
    """Determine the number of rows that can be made"""
...
def ship_hit(ai_settings, stats, screen, ship, aliens, bullets):
    ...
    if stats.ships_left > 0:
        # Decrement shhips left.
        stats.ships_left -= 1
        # Empty the list of aliens and bullets.
        aliens.empty()
        bullets.empty()

        # Create a new fleet and center the shihp
        create_fleet(ai_settings, screen, ship, aliens)
        ship.center_ship()
...
def check_aliens_bottom(ai_settings, stats, screen, ship, aliens, bullets):
    """Check if any aliens hit bottom"""
    screen_rect = screen.get_rect()
    for alien in aliens.sprites():
        if alien.rect.bottom >= screen_rect.bottom:
            # Treat this as the ship getting hit
...
def update_screen(ai_settings, screen, ship, aliens, bullets):
    ...
    ship.blitme()
    aliens.draw(screen) # Pygame draws each element in the group
...
def update_aliens(ai_settings, stats, screen, ship, aliens, bullets):
    """Check if the fleet is at theh edge, thhen update the positions"""
